...
if __name__ == "__main__":
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Odpowiadaj na pytania krótko i zwięźle. "
                "Zwóć odpowiedź jeżeli jesteś pewnien, że ją znasz. "
                "Nie znasz odpowiedzi albo jest za mało danych, zwróć tylko słowo NIE",
            ),
    ...
    task = TaskApi("whoami")
    found = False
    msgs = [HumanMessage(content="Podaje listę hintów:")]
    while found is False:
        data = task.task()
        msgs.append(HumanMessage(content=f" hint: {data['hint']}"))
        ret = chain.invoke(
            {"messages": msgs + [HumanMessage(content=f"Co to za osoba?")]}
        )
        print(ret)
        found = True if ret.content != "NIE" else False
    assert task.answer(ret.content)
